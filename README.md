...
Authors: Reigo Hendrikson, Roel M. Hogervorst , and Suthira
Owlarn
...
are good (❤️) or bad (🌩). Let’s call that goodness\[1\].
...
want to be pet. Suzie and Donald don’t want to be cuddled. So let’s
...
Before showing the purrr solution we remove `Food` and `Petting` from
cats (this is necessary evil, of if this is really upsetting to you, we
wait for half a day before starting again).
